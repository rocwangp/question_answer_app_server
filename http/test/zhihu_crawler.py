...
def answer_splider():
    ...
    ids = ["19553176", "19553298", "19559052", "19564606", "19567962", "19550429", "19552706", "19551077", "19564876"]
    re_url = r'((ht|f)tps?):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%&:\/~\+#]*[\w\-\@?^=%&\/~\+#])?'
    for id in ids:
        spider_page = 1;
        url = "https://www.zhihu.com/topic/" + id + "/top-answers?page="
        while spider_page < 10:
            ...
            for question in soup.findAll(attrs={"class":"question_link"}):
                ...
                question_browser = webdriver.PhantomJS()
                question_browser.get(question_url)
                question_soup = BeautifulSoup(question_browser.page_source, 'lxml')

                question_title = question_soup.findAll(attrs={"class":"QuestionHeader-title"})[0]
                ...
                f.write("\r")
                f.write("\n")
                question_detail = question_soup.findAll(attrs={"class":"QuestionHeader-detail"})[0].findAll(attrs={"class":"RichText"})
                if question_detail:
